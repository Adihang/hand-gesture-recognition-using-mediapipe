    ...
    for file in files:
        if file.endswith(".json"):
            ...
            if int(lo_value) not in filter_lo_value or drug != '일반의약품':
                ...
                image_file_path = image_file_path.replace(label_files_path, image_files_path)
                image_file_path = image_file_path.replace('_json', '')
                image_file_path = image_file_path.replace('TL', 'TS')
                try:
                # 해당 JSON 파일 삭제
                    os.remove(json_file_path)
                    #print(f'{json_file_path} 삭제되었습니다.')
                except:
