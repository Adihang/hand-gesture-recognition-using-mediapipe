...
def filter_boxes_by_bottom_region(labels, xyxys, bottom_region):
    filtered_boxes = []
...
if orig_image.shape[1] > 640:
    image = resize_img(orig_image,orig_path)
else:
    image = orig_image
...
filtered_boxes = filter_boxes_by_bottom_region(label_list, xyxy_list, bottom_region)

# 선택된 bounding box들을 이미지에 그리기
draw_boxes(image, filtered_boxes)
